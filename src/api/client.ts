...
export const calendarApi = {
  async getAll(person: 'ja' | 'jo') {
    const res = await fetch(`${getApiBaseUrl()}/api/calendar?person=${person}`);
  ...
  async update(person: 'ja' | 'jo', entries: any) {
    ...
    if (!res.ok) throw new Error(`Failed to update calendar: ${res.status}`);
    return res.json();
  },

  async updateEntry(person: 'ja' | 'jo', date: string, entry: any) {
    const res = await fetch(`${getApiBaseUrl()}/api/calendar?person=${person}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ date, entry }),
    });
    if (!res.ok) throw new Error(`Failed to update calendar entry: ${res.status}`);
    return res.json();
  },

  async deleteEntry(person: 'ja' | 'jo', date: string) {
    const res = await fetch(`${getApiBaseUrl()}/api/calendar?person=${person}&date=${date}`, {
      method: 'DELETE',
    });
    if (!res.ok) throw new Error(`Failed to delete calendar entry: ${res.status}`);
    return res.json();
  },
};
