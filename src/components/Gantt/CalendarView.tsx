...
export const CalendarView = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      try {
        // 先從 localStorage 載入作為備份（向後相容）
        const localJa = localStorage.getItem(PERSON_CONFIG.ja.storageKey);
        const localJo = localStorage.getItem(PERSON_CONFIG.jo.storageKey);
        if (localJa) setDailyEntriesJa(JSON.parse(localJa));
        if (localJo) setDailyEntriesJo(JSON.parse(localJo));

        // 然後從雲端載入（會覆蓋本地資料）
        const [entriesJa, entriesJo] = await Promise.all([
          calendarApi.getAll('ja'),
          calendarApi.getAll('jo'),
        ]);

        setDailyEntriesJa(entriesJa);
        setDailyEntriesJo(entriesJo);

        // 同步到 localStorage 作為快取
        localStorage.setItem(PERSON_CONFIG.ja.storageKey, JSON.stringify(entriesJa));
        localStorage.setItem(PERSON_CONFIG.jo.storageKey, JSON.stringify(entriesJo));
      } catch (error) {
        console.error('載入日曆資料失敗:', error);
        // 如果雲端載入失敗，保留 localStorage 的資料
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // 儲存 Ja 資料到雲端
  useEffect(() => {
    if (Object.keys(dailyEntriesJa).length > 0 && !isLoading) {
      const saveData = async () => {
        setIsSyncing(true);
        try {
          await calendarApi.update('ja', dailyEntriesJa);
          // 同步到 localStorage 作為快取
          localStorage.setItem(PERSON_CONFIG.ja.storageKey, JSON.stringify(dailyEntriesJa));
        } catch (error) {
          console.error('儲存 JA 日曆資料失敗:', error);
          // 即使雲端儲存失敗，仍然保存到 localStorage
          localStorage.setItem(PERSON_CONFIG.ja.storageKey, JSON.stringify(dailyEntriesJa));
        } finally {
          setIsSyncing(false);
        }
  ...
  }, [dailyEntriesJa, isLoading]);

  // 儲存 Jo 資料到雲端
  useEffect(() => {
    if (Object.keys(dailyEntriesJo).length > 0 && !isLoading) {
      const saveData = async () => {
        setIsSyncing(true);
        try {
          await calendarApi.update('jo', dailyEntriesJo);
          // 同步到 localStorage 作為快取
          localStorage.setItem(PERSON_CONFIG.jo.storageKey, JSON.stringify(dailyEntriesJo));
        } catch (error) {
          console.error('儲存 JO 日曆資料失敗:', error);
          // 即使雲端儲存失敗，仍然保存到 localStorage
          localStorage.setItem(PERSON_CONFIG.jo.storageKey, JSON.stringify(dailyEntriesJo));
        } finally {
          setIsSyncing(false);
